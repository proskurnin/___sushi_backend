...
// Переменные recipe_type, recipe_group, recipe_view
if (isset($_POST['recipe_type'])) {$recipe_type = $_POST['recipe_type'];} else {$recipe_type = -1;}
...
//             Это блок №2 - "данные"
            ...
            else
            {
                ...
                (`{$tabname}_id`,`{$tabname}_name`,`recipe_type`,`recipe_group`,`recipe_view`,`component_name`,`component_gross`,`component_net`,`component_comment`)
                VALUES  (NULL,'$add_name','$recipe_type','$recipe_group','$recipe_view','$component_name','$component_gross','$component_net','$component_comment')";
                /* Выполняем запрос. Если произойдет ошибка - вывести ее. */
                $mysqli->query($query) or die($query);
            }
...
// Заголовок поля формы добавления и редактирования данных
            ...
            if ($showtype == 'show_edit')
            {
                ...
                while (($editrr = $editquery->fetch_assoc()) != false)
                {
                    ...
                    {
                        $editrecipe_type = $editrr['recipe_type'];
                            ...
                            <div class='container'>
                                ...
                                <div class='row'>
                                    <div class='col-xs-4'>
                                        ...
                                        $recipe_type_list = '';
                                        $query = "SELECT * FROM `recipes_types` ORDER BY recipe_type ASC";
                                        $sel_recipe_type = $mysqli->query($query) or die($query);
                                        while (($opt_recipe_type = $sel_recipe_type->fetch_assoc()) != false)
                                        {
                                            if ($opt_recipe_type['recipe_type'] !== $editrecipe_type)
                                            {
                                                $recipe_type_list = $recipe_type_list.'<option value="'.$opt_recipe_type['recipe_type'].'">'.$opt_recipe_type['recipe_type'].'</option>';
                                            }
                                        }


                    echo "
                                            <select class='form-control form-control-warning selectpicker' id='recipe_type' name='recipe_type' required>
                                                <option value='{$editrecipe_type}'>{$editrecipe_type}</option>
            ...
            else if ($showtype == 'show_mistake')
            {
                ...
                echo "
                    ...
                    echo "
                                            <select class='form-control form-control-success selectpicker' id='recipe_type' name='recipe_type' required>
                                                <option value='{$editrecipe_type}'>{$editrecipe_type}</option>
                                                <optgroup label='Другие возможные типы:'>
                                                    {$recipe_type_list}
                                                </optgroup>
            ...
            else if ($showtype == 'show_manag' || isset($_GET['del']))
            {
                ...
                if ($event == 'event_first')
                {
                    ...
                    echo "
                                            <select class='form-control selectpicker' id='recipe_type' name='recipe_type' required>
                                                <option value=''>Укажите ТИП рецепта</option>
                    ...
                    echo "
                                            <select class='form-control selectpicker' id='recipe_group' name='recipe_group' required>
                                                <option value=''>Укажите ГРУППУ рецепта</option>
                                                <optgroup label='Возможные группы:'>
                                                    {$recipe_group_list}
                                                </optgroup>
                                            </select>
                                        </div>
                    ...
                    echo "
                                            <select class='form-control selectpicker' id='recipe_view' name='recipe_view' required>
                                                <option value=''>Укажите ВИД рецепта</option>
                                                <optgroup label='Возможные виды:'>
                                                    {$recipe_view_list}
                                                </optgroup>
                                            </select>
                                        </div>
...
// Режим просмотра таблицы
            ...
            группы:
            <a name='group' href=" . $urlform . "?group=all> 
                все
            </a>
             : 
            <a name='group' href=" . $urlform . "?group=klass> 
                классика
            </a>
             : 
            <a name='group' href=" . $urlform . "?group=firm> 
                фирменные
            </a><br>
            
            типы:
            <a name='type' href=" . $urlform . "?type=all> 
                все
            </a>
             : 
            <a name='type' href=" . $urlform . "?type=rolly> 
                роллы
            </a>
             : 
            <a name='type' href=" . $urlform . "?type=sushi> 
                суши
            </a>
             :
            <a name='type' href=" . $urlform . "?type=woki> 
                воки
            </a>
             : 
            <a name='type' href=" . $urlform . "?type=salat> 
                салаты
            </a>
             : 
            <a name='type' href=" . $urlform . "?type=soup> 
                супы
            </a>
             :
            <a name='type' href=" . $urlform . "?type=pizza> 
                пицца
            </a>
             : 
            <a name='type' href=" . $urlform . "?type=sous> 
                соусы
            </a><br>
            
            
            виды:
            <a name='view' href=" . $urlform . "?view=all> 
                все
            </a>
             : 
            <a name='view' href=" . $urlform . "?view=base> 
                базовый
            </a>
             : 
            <a name='view' href=" . $urlform . "?view=hot> 
                острый
            </a>
             :
            <a name='view' href=" . $urlform . "?view=vegeto> 
                вегето
            </a>
             :
            <a name='view' href=" . $urlform . "?view=warm> 
                горячий
            </a>
            <br>
            <br>
            ...
            // Создаём массив для работы с ним
            if(
                $_GET['group'] == 'all' || $_GET['group'] == '' || $_GET['group'] == 0 ||
                $_GET['type'] == 'all' || $_GET['type'] == '' || $_GET['type'] == 0 ||
                $_GET['view'] == 'all' || $_GET['view'] == '' || $_GET['view'] == 0
            )
            {
                $query = "SELECT * FROM `{$tabname}s` ORDER BY `{$tabname}_name` ASC";
            }

            if($_GET['group'] == 'klass')
            {
                $query = "SELECT * FROM `{$tabname}s` WHERE `{$tabname}_group` = 'классика' ORDER BY `{$tabname}_name` ASC";
            }
            if($_GET['group'] == 'firm')
            {
                $query = "SELECT * FROM `{$tabname}s` WHERE `{$tabname}_group` = 'фирменные' ORDER BY `{$tabname}_name` ASC";
            }


            if($_GET['type'] == 'rolly')
            {
                $query = "SELECT * FROM `{$tabname}s` WHERE `{$tabname}_type` = 'роллы' ORDER BY `{$tabname}_name` ASC";
            }
            if($_GET['type'] == 'sushi')
            {
                $query = "SELECT * FROM `{$tabname}s` WHERE `{$tabname}_type` = 'суши' ORDER BY `{$tabname}_name` ASC";
            }
            if($_GET['type'] == 'woki')
            {
                $query = "SELECT * FROM `{$tabname}s` WHERE `{$tabname}_type` = 'воки' ORDER BY `{$tabname}_name` ASC";
            }
            if($_GET['type'] == 'salat')
            {
                $query = "SELECT * FROM `{$tabname}s` WHERE `{$tabname}_type` = 'салаты' ORDER BY `{$tabname}_name` ASC";
            }
            if($_GET['type'] == 'soup')
            {
                $query = "SELECT * FROM `{$tabname}s` WHERE `{$tabname}_type` = 'супы' ORDER BY `{$tabname}_name` ASC";
            }
            if($_GET['type'] == 'pizza')
            {
                $query = "SELECT * FROM `{$tabname}s` WHERE `{$tabname}_type` = 'пицца' ORDER BY `{$tabname}_name` ASC";
            }
            if($_GET['type'] == 'sous')
            {
                $query = "SELECT * FROM `{$tabname}s` WHERE `{$tabname}_type` = 'соусы' ORDER BY `{$tabname}_name` ASC";
            }


            // Фильтр по видам роддлв
            if($_GET['view'] == 'base')
            {
                $query = "SELECT * FROM `{$tabname}s` WHERE `{$tabname}_view` = 'базовый' ORDER BY `{$tabname}_name` ASC";
            }
            if($_GET['view'] == 'hot')
            {
                $query = "SELECT * FROM `{$tabname}s` WHERE `{$tabname}_view` = 'острый' ORDER BY `{$tabname}_name` ASC";
            }
            if($_GET['view'] == 'vegeto')
            {
                $query = "SELECT * FROM `{$tabname}s` WHERE `{$tabname}_view` = 'вегето' ORDER BY `{$tabname}_name` ASC";
            }
            if($_GET['view'] == 'warm')
            {
                $query = "SELECT * FROM `{$tabname}s` WHERE `{$tabname}_view` = 'горячий' ORDER BY `{$tabname}_name` ASC";
            }
