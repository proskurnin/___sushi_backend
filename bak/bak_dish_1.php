...
//
if ($_POST['newadd'] == 'newadd' || $event == 'editdoc')
{
...
<div class="container">
    <div class="row">
        ...
        <form action="<?php echo $urlform ?>" method="post">
            ...
            <input type="hidden" name="editrow" value="0">
        ...
        echo '$edit_name = '.$edit_name.'<br>';
        echo '$recipe_name = '.$recipe_name.'<br>';
        echo '$_POST[showtype] = '.$_POST[showtype].'<br>';
        echo '$_POST[num_rows] = '.$_POST[num_rows].'<br>';
        echo '$_POST[add] = '.$_POST[add].'<br>';
        echo '$_POST[newadd] = '.$_POST[newadd].'<br>';
        echo '$num_rows = '.$num_rows.'<br>';
        echo '$event = '.$event.'<br>';



        //            if ($_POST['showtype'] == 'dataedit' || $event == 'editdoc' || isset($_GET['del']))
        if ($_POST['showtype'] == 'dataedit')
        {
            if ($_POST['add'] == 'add') {
                ...
                if ($event == 'editdoc')
                {
                    ...
                    echo "<button type='submit' name='newadd' value='newadd' class='btn btn-outline-warning pull-right'>Внести изменение</button>";
                }
                // Отображение ПЕРВОГО внесения данных
                else
                {
                    echo "<input type='hidden' name='num_rows' value='".$num_rows."'>";
                    echo "<button type='submit' name='newadd' value='newadd' class='btn btn-outline-primary pull-right'>Добавить рецепт</button>";
                }
                echo "</div>";
            ...
            echo "
                ...
                <form action='".$urlform."' method='post' class='form-inline' >
                    <input type='hidden' name='showtype' value='dataedit'>
                    <label for='num_rows'>Количество составляющих: </label>
                    <input type='number' value='5' class='form-control' id='num_rows' name='num_rows' min='1' max='20' size='5'>
                    <button type='submit' name='add' value='add' class='btn btn-outline-success'>Добавить данные</button>
                </form>
...
// Режим просмотра таблицы
	    ...
	    <br>
            <form class='form-inline' action='".$urlform."' method='post'>
				<input type='password' placeholder='Пароль для редактирования' class='form-control' id='passforedit' name='passforedit'>
                <button type='submit' name='showtype' value='dataedit' class='btn btn-outline-success'>Редактировать данные</button>
            </form>
            ";
