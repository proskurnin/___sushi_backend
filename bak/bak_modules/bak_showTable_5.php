...
while (($row = $result_set->fetch_assoc()) != false) {
    ...
    $query_dish = "SELECT * FROM `dishs` WHERE  `dish_name` = '".$row['component_name']."'";
    ...
    foreach ($row as $k => $v) {
        ...
        if (($k == $tabname.'_name') && ($v == $item_name)) {
            ...
            else
            {
                ...
                $dbdish = $mysqli->query($query_dish) or die($query_dish);
                /* определение числа рядов в выборке рецептов */
                $row_dish_cnt = mysqli_num_rows($dbdish);
