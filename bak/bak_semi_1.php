...
function rowform ($i,$event,$edit_name) {
    if ($event == 'event_first') {
        ...
        <!-- Количество компонента БРУТТО -->
            <div class='col-xs-2'>
                <div class='form-group'>              
                    <input type='text' placeholder='Брутто' class='col-xs-2 form-control' id='gross_{$i}' name='gross_{$i}' required>
                </div>
            </div>
        ...
        <!-- Количество компонента НЕТТО -->
            <div class='col-xs-2'>
                <div class='form-group'>
                    <input type='text' placeholder='Нетто' class='col-xs-2 form-control' id='net_{$i}' name='net_{$i}' required>
                </div>
            </div>
...
//      КОНЕЦ ДИАГНОСТИКИ
    ...
    if ($event == 'event_edit') {
        ...
        $db_components = $mysqli->query("SELECT  `product_id` FROM  `semis` WHERE  `semi_name` =  '$edit_name'") or die();
        while (($db_element_component = $db_components->fetch_assoc()) != false)
        {
            $element_component_db[] = array_shift($db_element_component);
        }
        $db_gross = $mysqli->query("SELECT  `product_gross` FROM  `semis` WHERE  `semi_name` =  '$edit_name'") or die();
        while (($db_element_gross = $db_gross->fetch_assoc()) != false)
        {
            $element_gross_db[] = array_shift($db_element_gross);
        }
        $db_net = $mysqli->query("SELECT  `product_net` FROM  `semis` WHERE  `semi_name` =  '$edit_name'") or die();
        while (($db_element_net = $db_net->fetch_assoc()) != false)
        {
            $element_net_db[] = array_shift($db_element_net);
        }
        $db_comment = $mysqli->query("SELECT  `component_comment` FROM  `semis` WHERE  `semi_name` =  '$edit_name'") or die();
...
//        echo '<br>';
//        echo $semi_name_check . " - - - - - - - - - ";
//        echo $row['semi_name'] . "<br>";
        if ($semi_name_check !== $row['semi_name'])
        {
            if ($npp !== 0)
            {
                echo "
                    ...
                    <td>" . $semi_protein . "</td>
                    <td>" . $semi_fat . "</td>
                    <td>" . $semi_carbohydrate . "</td>
                    <td>" . $semi_energy . "</td>
                    <td>
                    <a name='del' href=".$urlform."?del=".urlencode($semi_name).">
                    <i class='fa fa-times' aria-hidden='true' style='color: red;'></i>
                    </a>
            
                    &nbsp;&nbsp;&nbsp;
            
                    <a name='edit' href=".$urlform."?editrow=editrow&edit_name=".urlencode($semi_name).">
        ...
        $semi_name_check = $row['semi_name'];

        foreach ($row as $k => $v) {
//            echo '$k ('.$k.') = '.($k == 'semi_name').' ';
//            echo '$v = '.($v == $semi_name).'<br>';
            if (($k == 'semi_name') && ($v == $semi_name)) {
                ...
                $semi_amount = $semi_amount + ($row["product_net"]);

//                    echo $row['product_id'] . ": ";
//                    echo $semi_cost . ' = ' . $semi_cost . ' + (' . $row["product_gross"] . ' * ' . $product["price_online"] . ')<br>';
                $semi_cost = $semi_cost + ($row["product_gross"] * $product["price_online"]);
//                    echo "$$$ " . $semi_cost . "<br>";

                $semi_protein = $semi_protein + ($row["product_net"] * $product["protein"]);
                $semi_fat = $semi_fat + ($row["product_net"] * $product["fat"]);
                $semi_carbohydrate = $semi_carbohydrate + ($row["product_net"] * $product["carbohydrate"]);
                $semi_energy = $semi_energy + ($row["product_net"] * $product["energy"]);
            }
        }
    }
    if ($npp !== 0)
...
function rowdelete ($del) {
    $mysqli = new mysqli("localhost", "u0216880_sushi", "02oL1NdiCm", "u0216880_sushi");
    $mysqli->query("SET NAMES 'utf8'");
    $query = "DELETE FROM `semis` WHERE `semi_name` = '$del'";
...
function rowscounter ($row_name){
    $mysqli = new mysqli("localhost", "u0216880_sushi", "02oL1NdiCm", "u0216880_sushi");
    $mysqli->query("SET NAMES 'utf8'");
    $query = "SELECT  `product_id` FROM  `semis` WHERE  `semi_name` =  '$row_name'";
    /* Выполняем запрос. Если произойдет ошибка - вывести ее. */
...
// 
//  ДОДЕЛАТЬ ВКЛЮЧЕНИ И ВЫКЛЮЧЕНИЕ ДИАГНОСТИКИ
//
...
//       Добавление данных в таблицу semis
//
...
// Обработчик ошибок - поиск незаполненных полей
    ...
    $query = "SELECT `semi_name` FROM `semis`";
    $db_names = $mysqli->query($query) or die($query);
    while (($db_semis_names = $db_names->fetch_assoc()) != false)
    {
        if ($add_name == $db_semis_names['semi_name']) {
            $error = 1;
            $dubl = 1;
...
//        $add_name = $edit_name;
        ...
        $db_components = $mysqli->query("SELECT  `semi_id` FROM  `semis` WHERE  `semi_name` =  '$edit_name'") or die();
        while (($db_element_component = $db_components->fetch_assoc()) != false)
        {
            $semi_id_arr[] = $db_element_component['semi_id'];
        }
...
//             Это блок №2 - "данные"
            ...
            echo 'ID строки рецепта: <span style="color: red;">'.$semi_id.'</span> <b>||</b> ';
...
//             Это блок №2 - "данные"
            ...
            $query = "INSERT INTO `semis`
            (`semi_id`,`semi_name`,`product_id`,`product_gross`,`product_net`,`component_comment`)
...
//             и таблицу с данными для редактирвоания
    ...
    <form action='".$urlform."' class='form-horizontal' method='post'>
        ...
        <!-- Название рецепта -->
            ...
            // Строки в режиме event_edit
            if ($event == 'event_edit')
            {
                echo "<div class='form-group has-warning'>";
                echo "<input type='text' value='".$name."' class='col-xs-3 form-control form-control-warning' id='add_name_edit' name='add_name_edit' required>";
                echo "</div>";
            }
            ...
            // Строки в режиме event_first
            else if ($event == 'event_first')
            {
                echo "<div class='form-group'>";
                echo "<input type='text' placeholder='Название рецепта' class='col-xs-3 form-control' id='add_name' name='add_name' required>";
                echo "</div>";
            }
            ...
            else if (!isset($_POST['add_name']) || $_POST['add_name'] == '0' || $_POST['add_name'] == '' || $dubl == 1)
            {
                ...
                echo "<input type='text' placeholder='Название рецепта' class='col-xs-3 form-control form-control-danger' id='add_name' name='add_name' required>";
                echo "</div>";
            }
            ...
            else
            {
                ...
                echo "<input type='text' value='".$name."' class='col-xs-3 form-control form-control-success' id='add_name' name='add_name' required>";
                echo "</div>";
            }
            echo "</div>";
            echo "</div>";
...
// на жёлтых полях радактирвоания
                ...
                echo "<input type='hidden' name='showtype' value='show_edit'>";
...
//
            ...
            $semis_result_set = $mysqli->query("SELECT * FROM `semis`");
            echo editTable ($semis_result_set);
...
// Режим просмотра таблицы
            ...
            echo showTable ($semis_result_set);
