            ...
            else
            {
                ...
                <div class='col-xs-4'>
                    ...
                    <select class='form-control form-control-success' id='component_name_{$i}' name='component_name_{$i}' required>
                        <option value='".$_POST['component_name_'.$i]."'>".$_POST['component_name_'.$i]."</option>
                        <option value='0'>- не использовать -</option>
                        <optgroup label='Сырьё и упаковка'>
                            {$options_products}
                        </optgroup>
...
function editTable ($result_set,$tabname)
{
    ...
    while (($row = $result_set->fetch_assoc()) != false) {
        ...
        $query = "SELECT * FROM `products` WHERE `product_name` = '".$row['component_name']."'";
        ...
        foreach ($row as $k => $v) {
            if (($k == $tabname.'_name') && ($v == $item_name)) {
                ...
                $item_cost = $item_cost + ($row["component_gross"] * $product["price_online"]);
...
function showTable ($result_set,$tabname,$nutr_val)
{
...
//                echo '<b style="color: red">$row["component_name"]</b><span style="color: red"> '.$row["component_name"].'</span><br>';

                if ($nutr_val == '1') {$nutr = $row["component_net"];} else {$nutr = 0.1;}

//                echo '<b>$item_amount</b> '.$item_amount.' + '.$row["component_net"].' = ';
                $item_amount = $item_amount + $row["component_net"];
//                echo $item_amount.'<br>';

//                echo '<b>$item_cost</b> '.$item_cost.' + ('.$row["component_gross"].' * '.$product["price_online"].') = ';
                $item_cost = $item_cost + ($row["component_gross"] * $product["price_online"]);
//                echo $item_cost.'<br>';

//                echo '<b>$item_protein</b> '.$item_protein.' + ('.$row["component_net"].' * '.$product["protein"].' / '.$nutr.') = ';
                $item_protein = $item_protein + ($row["component_net"] * $product["protein"] / $nutr);
//                echo $item_protein.'<br>';

//                echo '<b>$item_fat</b> '.$item_fat.' + ('.$row["component_net"].' * '.$product["fat"].' / '.$nutr.') = ';
                $item_fat = $item_fat + ($row["component_net"] * $product["fat"] / $nutr);
//                echo $item_fat.'<br>';

//                echo '<b>$item_carbohydrate</b> '.$item_carbohydrate.' + ('.$row["component_net"].' * '.$product["carbohydrate"].' / '.$nutr.') = ';
                $item_carbohydrate = $item_carbohydrate + ($row["component_net"] * $product["carbohydrate"] / $nutr);
//                echo $item_carbohydrate.'<br>';

//                echo '<b>$item_energy</b> '.$item_energy.' + ('.$row["component_net"].' * '.$product["energy"].' / '.$nutr.') = ';
                $item_energy = $item_energy + ($row["component_net"] * $product["energy"] / $nutr);
//                echo $item_energy.'<br>';
            }
        }
    }
...
// Режим просмотра таблицы
            ...
            echo showTable ($items_result_set,$tabname,$nutr_val);
