...
//      КОНЕЦ ДИАГНОСТИКИ
    ...
    if ($event == 'event_edit') {
        ...
        $query = "SELECT  `product_id` FROM  `semis` WHERE  `semi_name` =  '$edit_name'";
        ...
        $query = "SELECT  `product_gross` FROM  `semis` WHERE  `semi_name` =  '$edit_name'";
        ...
        $query = "SELECT  `product_net` FROM  `semis` WHERE  `semi_name` =  '$edit_name'";
        ...
        $query = "SELECT  `component_comment` FROM  `semis` WHERE  `semi_name` =  '$edit_name'";
...
//    Функция удаления данных из таблицы semis
...
if (isset($_GET['edit_name']) || isset($_POST['edit_name'])) {$num_rows = rowscounter($edit_name);}
...
while (($db_semis_names = $db_names->fetch_assoc()) != false) {
    ...
    echo 'add_name - '.($_POST['add_name'] == $db_semis_names['semi_name']).'<br>';
    echo 'add_name_edit - '.($_POST['add_name_edit'] == $db_semis_names['semi_name']).'<br>';
    echo 'add_name_mistake - '.($_POST['add_name_mistake'] == $db_semis_names['semi_name']).'<br>';
    echo 'add/edit - '.($add_name !== $edit_name).'<br>';
    echo '$add_name = '.$add_name.'<br>';

    echo 'results - '.((($_POST['add_name'] == $db_semis_names['semi_name']) ||
                ($_POST['add_name_edit'] == $db_semis_names['semi_name']) ||
                ($_POST['add_name_mistake'] == $db_semis_names['semi_name']))
                &&
                ($add_name !== $edit_name)
                &&
                ($add_name !== '')
    ...
    if ((($_POST['add_name'] == $db_semis_names['semi_name']) ||
        ($_POST['add_name_edit'] == $db_semis_names['semi_name']) ||
        ($_POST['add_name_mistake'] == $db_semis_names['semi_name']))
        &&
        ($add_name !== $edit_name)
...
// Нужно переделать через UPDATE
        ...
        $query = "SELECT  `semi_id` FROM  `semis` WHERE  `semi_name` =  '$edit_name'";
        $db_components = $mysqli->query($query) or die($query);
        while (($db_element_component = $db_components->fetch_assoc()) != false)
        {
...
//            if ()
            ...
            if ($product_id == '0')
            {
                ...
                $query = "DELETE FROM `semis` WHERE `semi_id` = '$semi_id'";
                /* Выполняем запрос. Если произойдет ошибка - вывести ее. */
                $mysqli->query($query) or die($query);
