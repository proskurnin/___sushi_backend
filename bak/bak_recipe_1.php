...
// Строим выпадающий список продуктов. Выбор продуктов в списке по ID
        $options_products = '';
...
//      КОНЕЦ ДИАГНОСТИКИ
        ...
        {} else {
            ...
            // Определяем выбранный пункт для строки
            if (substr($component_id,0,7) == 'product') {
            ...
            else
            {
                while (($optsemis = $selsemis->fetch_assoc()) != false)
                {
                    if ($component_id == $optsemis['semi_name'])
                    {
                        $select_option = $optsemis['semi_name'];
                    }
                }
            }

            // Создаём списки для выбора
...
function rowscounter ($row_name){
    ...
    $query = "SELECT  `recipe_id` FROM  `recipes` WHERE  `recipe_name` =  '$row_name'";
...
// Обработчик ошибок - поиск незаполненных полей
    ...
    for ($i = 0; $i++ < $num_rows;) {
        ...
        if (isset($_POST['component_id_'.$i]) && $_POST['component_id_'.$i] !== '0')
        {
            ...
            echo '$_POST[\'component_id_\'.'.$i.'] = '.$_POST['component_id_'.$i].'<br>';
...
//        }
    ...
    $count_names_arr = array_count_values($names_arr);
    foreach($count_names_arr as $kk=>$vv) {
        if ($vv > 1) {
            $error = 1;
...
// Нужно переделать через UPDATE
        ...
        // прописать условие, чтобы имя правильно влияло на количество строк



//        if (isset($_GET['edit_name'])) $num_rows = rowscounter($edit_name);
//        $add_name = $edit_name;
...
//             Это блок №2 - "данные"
            ...
            echo 'Продукт ID: <span style="color: red;">'.$product_id.'</span> <b>||</b> ';
            echo 'Брутто: <span style="color: red;">'.$product_gross.'</span> <b>||</b> ';
            echo 'Нетто: <span style="color: red;">'.$product_net.'</span> <b>||</b> ';
            echo 'Полуфабрикат NAME: <span style="color: red;">'.$semi_name.'</span> <b>||</b> ';
            echo 'Брутто: <span style="color: red;">'.$semi_gross.'</span> <b>||</b> ';
            echo 'Нетто: <span style="color: red;">'.$semi_net.'</span> <b>||</b> ';
...
//            if ()
            if (substr($component_id,0,7) == 'product')
            {
                $product_id = trim($component_id, 'product_');
                $product_gross = $_POST['gross_'.$i];
                $product_net = $_POST['net_'.$i];
                $semi_name = '0';
                $semi_gross = '0';
            ...
            {
                $semi_name = $_POST['semi_name_'.$i];
                $semi_gross = $_POST['gross_'.$i];
                $semi_net = $_POST['net_'.$i];
                $product_id = '0';
                $product_gross = '0';
                $product_net = '0';
            }
...
//             Это блок №2 - "данные"
            ...
            (`recipe_id`,`recipe_name`,`product_id`,`product_gross`,`product_net`,`semi_name`,`semi_gross`,`semi_net`,`component_comment`)
            VALUES  (NULL,'$add_name','$product_id','$product_gross','$product_net','$semi_name','$semi_gross','$semi_net','$component_comment')";
            /* Выполняем запрос. Если произойдет ошибка - вывести ее. */
            $mysqli->query($query) or die($query);
        }
...
//             и таблицу с данными для редактирвоания
        ...
        if ($showtype !== 'show_no' || isset($_GET['del']) || isset($_GET['edit_name']))
        {
            ...
            {
                if ($edit_name == '-1') $show_name = $_POST['add_name']; else $show_name = $edit_name;
                echo "<h4>Редактирование рецепта ".$show_name."</h4>";
